"""
PLEASE USE THIS FORM FOR AUTO PAIRING ONLY!
To use this form, rename it to models.py
"""
...
class Pair(models.Model):
    ...
    def save(self, *args, **kwargs): ## Overiding the save function of Pair
        if (((str(self.mentee)==None) or (str(self.mentee)) == "" or (str(self.mentor))==None or (str(self.mentor)) == "")):
            if(check(auto(mentor,mentee))==True):
...
def auto(mentee,mentor):
    menteeId = mentee.split(' ', 1)[1]
    mentorId = mentor.split(' ', 1)[1]

    if menteeId=="":
        mentorgender = Profile.objects.filter(uniId__contains=mentorId).values_list('gender',flat=True)[0]
        mentordegree =  Profile.objects.filter(uniId__contains=mentorId).values_list('degree_programme',flat=True)[0]
        mentorinteraction = Profile.objects.filter(uniId__contains=mentorId).values_list('medium_interaction',flat=True)[0]

        foundMentee = Profile.objects.filter(role == "Mentee",gender=mentorgender,mentordegree).values_list('uniId',flat=True)[0]
        return foundMentee,mentorId

    else mentorId=="":
        menteegender = Profile.objects.filter(uniId__contains=menteeId).values_list('gender',flat=True)[0]
        menteedegree =  Profile.objects.filter(uniId__contains=menteeId).values_list('degree_programme',flat=True)[0]
        menteeinteraction = Profile.objects.filter(uniId__contains=menteeId).values_list('medium_interaction',flat=True)[0]

        foundMentor = Profile.objects.filter(role == "Mentor",gender=menteegender,menteedegree).values_list('uniId',flat=True)[0]
        return foundMentor,menteeId
